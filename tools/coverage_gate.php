...
if (!is_readable($reportfile)) {
    fwrite(STDERR, "Clover report not readable: $reportfile\n");
    exit(1);
}

$xml = simplexml_load_file($reportfile);
if ($xml === false) {
    fwrite(STDERR, "Clover report could not be parsed.\n");
    exit(1);
}
...
if ($component === null) {
    fwrite(STDERR, "Component could not be read from version.php.\n");
    exit(1);
}
// A single plugin needs no lookup table: its install path is fixed at template initialisation.
$needle = '/local/stackmatheditor/';
if ($component !== 'local_stackmatheditor') {
    fwrite(STDERR, "Unexpected component: $component\n");
    exit(1);
}
echo "Component: $component (path filter: $needle)\n";

$covered = 0;
$total = 0;
foreach ($xml->xpath('//file') as $file) {
    $name = (string) $file['name'];
    // Restricted to this plugin: qtype_stack and core are installed alongside it, but their
...
if ($total === 0) {
    fwrite(STDERR, "No plugin files found in the report - the gate cannot be evaluated.\n");
    exit(1);
}

$pct = round($covered / $total * 100, 2);
printf("Line coverage (this plugin only): %s%% (%d/%d), floor %s%%\n", $pct, $covered, $total, $floor);
exit($pct < $floor ? 1 : 0);
