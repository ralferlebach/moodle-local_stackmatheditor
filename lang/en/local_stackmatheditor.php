<?php
// This file is part of Moodle - https://moodle.org/
//
// Moodle is free software: you can redistribute it and/or modify
// it under the terms of the GNU General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// Moodle is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// You should have received a copy of the GNU General Public License
// along with Moodle.  If not, see <https://www.gnu.org/licenses/>.

/**
 * English language strings for local_stackmatheditor.
 *
 * @package    local_stackmatheditor
 * @copyright  2026 Ralf Erlebach
 * @license    https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */

defined('MOODLE_INTERNAL') || die();

$string['aria_add_line'] = 'Add line';
$string['aria_add_row'] = 'Add equation row';
$string['aria_formula_input'] = 'Formula input (visual editor)';
$string['aria_remove_row'] = 'Remove equation row';
$string['aria_remove_step'] = 'Remove transformation step';
$string['back'] = 'Back';
$string['btn_Alpha'] = 'Alpha (Α)';
$string['btn_Beta'] = 'Beta (Β)';
$string['btn_Chi'] = 'Chi (Χ)';
$string['btn_Delta'] = 'Delta (Δ)';
$string['btn_Epsilon'] = 'Epsilon (Ε)';
$string['btn_Eta'] = 'Eta (Η)';
$string['btn_Gamma'] = 'Gamma (Γ)';
$string['btn_Iota'] = 'Iota (Ι)';
$string['btn_Kappa'] = 'Kappa (Κ)';
$string['btn_Lambda'] = 'Lambda (Λ)';
$string['btn_Mu'] = 'Mu (Μ)';
$string['btn_Nu'] = 'Nu (Ν)';
$string['btn_Omega'] = 'Omega (Ω)';
$string['btn_Omicron'] = 'Omicron (Ο)';
$string['btn_Phi'] = 'Phi (Φ)';
$string['btn_Pi'] = 'Pi (Π)';
$string['btn_Psi'] = 'Psi (Ψ)';
$string['btn_Rho'] = 'Rho (Ρ)';
$string['btn_Sigma'] = 'Sigma (Σ)';
$string['btn_Tau'] = 'Tau (Τ)';
$string['btn_Theta'] = 'Theta (Θ)';
$string['btn_Upsilon'] = 'Upsilon (Υ)';
$string['btn_Xi'] = 'Xi (Ξ)';
$string['btn_Zeta'] = 'Zeta (Ζ)';
$string['btn_abs'] = 'Absolute value |x|';
$string['btn_adjoint'] = 'Hermitian conjugate (A†)';
$string['btn_alpha'] = 'Alpha (α)';
$string['btn_angle'] = 'Angle (∠)';
$string['btn_angle_object'] = 'Angle ABC, with B as the vertex';
$string['btn_approx'] = 'Approximately equal (≈)';
$string['btn_arccos'] = 'Arccosine (acos)';
$string['btn_arcsin'] = 'Arcsine (asin)';
$string['btn_arctan'] = 'Arctangent (atan)';
$string['btn_beta'] = 'Beta (β)';
$string['btn_binomial'] = 'Binomial coefficient C(n,k)';
$string['btn_boltzmann'] = 'Boltzmann constant (k_B)';
$string['btn_cap'] = 'Intersection (∩)';
$string['btn_cdot'] = 'Multiplication (·)';
$string['btn_chi'] = 'Chi (χ)';
$string['btn_column_vector'] = 'Column vector with three rows';
$string['btn_complex'] = 'Complex numbers (ℂ)';
$string['btn_conjugate'] = 'Complex conjugate (A*)';
$string['btn_contour_integral'] = 'Contour integral (∮)';
$string['btn_cos'] = 'Cosine (cos)';
$string['btn_cross'] = 'Cross product (×)';
$string['btn_cup'] = 'Union (∪)';
$string['btn_curly_brackets'] = 'Curly brackets { }';
$string['btn_degree'] = 'Degree (°)';
$string['btn_delta'] = 'Delta (δ)';
$string['btn_deriv'] = 'Derivative (d/dx)';
$string['btn_determinant'] = 'Determinant det(A)';
$string['btn_distance'] = 'Distance between two points';
$string['btn_div'] = 'Division (÷)';
$string['btn_div_op'] = 'Divergence (div)';
$string['btn_electron_charge'] = 'Elementary charge (e⁻)';
$string['btn_epsilon'] = 'Epsilon (ε)';
$string['btn_equal'] = 'Equals (=)';
$string['btn_eta'] = 'Eta (η)';
$string['btn_euler'] = 'Euler\'s number (e = 2.718…)';
$string['btn_exists'] = 'There exists (∃)';
$string['btn_exp'] = 'Exponential function (exp)';
$string['btn_expected_value'] = 'Expected value E[X]';
$string['btn_factorial'] = 'Factorial (n!)';
$string['btn_forall'] = 'For all (∀)';
$string['btn_fraction'] = 'Fraction';
$string['btn_gamma'] = 'Gamma (γ)';
$string['btn_gamma_func'] = 'Gamma function (Γ)';
$string['btn_geq'] = 'Greater than or equal (≥)';
$string['btn_grad'] = 'Gradient (grad)';
$string['btn_gravitational'] = 'Gravitational constant (G)';
$string['btn_greater'] = 'Greater than (>)';
$string['btn_hbar'] = 'Reduced Planck constant (ℏ)';
$string['btn_iff'] = 'If and only if (⇔)';
$string['btn_imaginary'] = 'Imaginary unit (i)';
$string['btn_impliedby'] = 'Is implied by (⇐)';
$string['btn_implies'] = 'Implies (⇒)';
$string['btn_in'] = 'Element of (∈)';
$string['btn_infty'] = 'Infinity (∞)';
$string['btn_integers'] = 'Integers (ℤ)';
$string['btn_integral'] = 'Integral (∫ with limits, integrand and variable; leave the limits empty for an indefinite integral)';
$string['btn_iota'] = 'Iota (ι)';
$string['btn_kappa'] = 'Kappa (κ)';
$string['btn_lambda'] = 'Lambda (λ)';
$string['btn_laplacian'] = 'Laplace operator (Δ)';
$string['btn_leq'] = 'Less than or equal (≤)';
$string['btn_less'] = 'Less than (<)';
$string['btn_ln'] = 'Natural logarithm (ln)';
$string['btn_log'] = 'Logarithm (log)';
$string['btn_logical_and'] = 'Logical AND (∧)';
$string['btn_logical_or'] = 'Logical OR (∨)';
$string['btn_matrix_2x2'] = '2 × 2 matrix';
$string['btn_matrix_3x3'] = '3 × 3 matrix';
$string['btn_matrix_popup'] = 'Matrix: choose the number of rows and columns';
$string['btn_minus'] = 'Minus (−)';
$string['btn_mp'] = 'Minus-plus (∓)';
$string['btn_mu'] = 'Mu (μ)';
$string['btn_nabla'] = 'Nabla / del operator (∇)';
$string['btn_naturals'] = 'Natural numbers (ℕ)';
$string['btn_neg'] = 'Logical NOT (¬)';
$string['btn_neq'] = 'Not equal (≠)';
$string['btn_nexists'] = 'There does not exist (∄)';
$string['btn_norm'] = 'Norm / magnitude (‖v‖)';
$string['btn_notin'] = 'Not element of (∉)';
$string['btn_nthroot'] = 'n-th root (ⁿ√)';
$string['btn_nu'] = 'Nu (ν)';
$string['btn_omega'] = 'Omega (ω)';
$string['btn_omicron'] = 'Omicron (ο)';
$string['btn_overline'] = 'Line segment (overline)';
$string['btn_partial'] = 'Derivative (∂/∂x of a bracketed expression)';
$string['btn_partial_mixed'] = 'Mixed derivative (∂²/∂x∂y; the total order must equal the sum of the orders)';
$string['btn_partial_n'] = 'Higher derivative (∂ⁿ/∂xⁿ; the total order must match the order at x)';
$string['btn_percent'] = 'Percent (%)';
$string['btn_permeability'] = 'Magnetic permeability (μ₀)';
$string['btn_permittivity'] = 'Electric permittivity (ε₀)';
$string['btn_perp'] = 'Perpendicular (⊥)';
$string['btn_phi'] = 'Phi (φ)';
$string['btn_pi'] = 'Pi (π = 3.14159…)';
$string['btn_pi_greek'] = 'Pi (π)';
$string['btn_plus'] = 'Plus (+)';
$string['btn_pm'] = 'Plus-minus (±)';
$string['btn_point_2d'] = 'Point with two coordinates';
$string['btn_point_3d'] = 'Point with three coordinates';
$string['btn_power'] = 'Power (xⁿ)';
$string['btn_prod'] = 'Product (∏)';
$string['btn_psi'] = 'Psi (ψ)';
$string['btn_rationals'] = 'Rational numbers (ℚ)';
$string['btn_reals'] = 'Real numbers (ℝ)';
$string['btn_rho'] = 'Rho (ρ)';
$string['btn_rot'] = 'Curl / rotation (rot)';
$string['btn_round_brackets'] = 'Round brackets ( )';
$string['btn_row_vector'] = 'Row vector with three columns';
$string['btn_segment_length'] = 'Length of the segment AB';
$string['btn_setminus'] = 'Set difference (∖)';
$string['btn_sigma'] = 'Sigma (σ)';
$string['btn_sin'] = 'Sine (sin)';
$string['btn_speed_of_light'] = 'Speed of light (c₀)';
$string['btn_sqrt'] = 'Square root (√)';
$string['btn_square_brackets'] = 'Square brackets [ ]';
$string['btn_std_dev'] = 'Standard deviation (σ)';
$string['btn_subset'] = 'Proper subset of (⊂)';
$string['btn_subseteq'] = 'Subset of or equal to (⊆)';
$string['btn_sum'] = 'Sum (∑)';
$string['btn_supset'] = 'Proper superset of (⊃)';
$string['btn_supseteq'] = 'Superset of or equal to (⊇)';
$string['btn_tan'] = 'Tangent (tan)';
$string['btn_tau'] = 'Tau (τ)';
$string['btn_theta'] = 'Theta (θ)';
$string['btn_transpose'] = 'Transpose (Aᵀ)';
$string['btn_unity_matrix'] = 'Identity matrix (𝟙)';
$string['btn_upsilon'] = 'Upsilon (υ)';
$string['btn_vec'] = 'Vector arrow (v⃗)';
$string['btn_vector_popup'] = 'Vector: choose the dimension and orientation';
$string['btn_xi'] = 'Xi (ξ)';
$string['btn_zeta'] = 'Zeta (ζ)';
$string['cannotresolveqbeid'] = 'The question bank entry could not be resolved.';
$string['config_saved'] = 'Toolbar configuration saved.';
$string['configure'] = 'Configure MathQuill toolbar';
$string['configure_adaptivequiz'] = 'MathQuill default settings for this adaptive quiz';
$string['configure_adaptivequiz_heading'] = 'MathQuill default settings for adaptive quiz: {$a}';
$string['configure_adaptivequiz_note'] = 'These settings apply to all STACK questions in this adaptive quiz.';
$string['configure_editor'] = 'Configure editor';
$string['configure_enabled_checkboxlabel_adaptivequiz'] = 'Enable MathQuill editor for this adaptive quiz';
$string['configure_enabled_checkboxlabel_question'] = 'Enable MathQuill editor for this question';
$string['configure_enabled_checkboxlabel_quiz'] = 'Enable MathQuill editor for this quiz';
$string['configure_enabled_desc'] = 'Enable or disable the editor for this quiz or question. Overrides the parent-level setting.';
$string['configure_enabled_header'] = 'Plugin activation';
$string['configure_enabled_label'] = 'Enable MathQuill editor';
$string['configure_enabled_label_help'] = 'If the parent-level default is "enabled", you can disable the editor here for this quiz or question — and vice versa.';
$string['configure_enabled_locked_off'] = 'Globally disabled – this setting cannot be overridden here.';
$string['configure_enabled_locked_on'] = 'Globally enabled – this setting cannot be overridden here.';
$string['configure_enabled_parenthint_off'] = 'Parent default: disabled. The checkbox enables the editor for this quiz / question.';
$string['configure_enabled_parenthint_on'] = 'Parent default: enabled. The checkbox disables the editor for this quiz / question only.';
$string['configure_heading'] = 'MathQuill toolbar for: {$a}';
$string['configure_quiz'] = 'MathQuill default settings for this quiz';
$string['configure_quiz_heading'] = 'MathQuill default settings for quiz: {$a}';
$string['configure_quiz_nav'] = 'Set up STACK MathQuill Editor';
$string['configure_quiz_note'] = 'These settings apply as defaults to all STACK questions in this quiz. They can be overridden for individual questions.';
$string['configure_studenttoggle_checkboxlabel'] = 'Allow students to switch the editor on and off';
$string['configure_studenttoggle_label'] = 'Student switch';
$string['configure_studenttoggle_label_help'] = 'While answering, students see a switch that puts the editor away and brings the plain STACK input back. The option only applies where the editor is enabled, and a level above can take it away: what is switched off higher up cannot be switched on again here.';
$string['dependency_marker'] = '{$a} *';
$string['dependency_missing'] = '* This group needs "{$a->package}". Add this to the question variables of the STACK question: {$a->instruction} Until then the group stays hidden from students.';
$string['dependency_unknown'] = '* The requirements of this group could not be checked for this question. The group stays hidden from students.';
$string['derivative_operand_missing'] = 'Incomplete derivative: write the expression to differentiate inside the brackets.';
$string['derivative_order_mismatch'] = 'Derivative: the order in the numerator must equal the sum of the orders in the denominator.';
$string['derivative_variable_composite'] = 'Derivative: only single variables such as ∂x or ∂t are supported in the denominator.';
$string['diffop_operand_missing'] = 'A differential operator needs an operand in brackets, for example grad(f).';
$string['diffop_unavailable'] = 'This differential operator is not available on this site: no Maxima function is configured for it.';
$string['enabled_mode_0'] = 'Completely disabled (no override at quiz or question level)';
$string['enabled_mode_1'] = 'Completely enabled (no override at quiz or question level)';
$string['enabled_mode_2'] = 'Off by default – can be enabled per quiz or question';
$string['enabled_mode_3'] = 'On by default – can be disabled per quiz or question';
$string['error_maxdimension_number'] = 'Enter a whole number, or leave the field empty to inherit.';
$string['error_maxdimension_range'] = 'Enter a number between {$a->min} and {$a->max}, or leave the field empty to inherit.';
$string['group_absolute']               = 'Absolute value';
$string['group_analysis_operators']     = 'Calculus operators';
$string['group_basic_operators']        = 'Basic arithmetic';
$string['group_brackets']               = 'Brackets';
$string['group_comparators']            = 'Comparison operators';
$string['group_constants_math']         = 'Mathematical constants';
$string['group_constants_nature']       = 'Physical constants';
$string['group_differential_operators'] = 'Differential calculus';
$string['group_exponential_log']        = 'Exponential / logarithm';
$string['group_geometry']               = 'Geometry';
$string['group_greek_lower']            = 'Greek letters (lowercase)';
$string['group_greek_upper']            = 'Greek letters (uppercase)';
$string['group_hyperbolic']             = 'Hyperbolic functions';
$string['group_integral_operators']     = 'Integral calculus';
$string['group_logic']                  = 'Logic';
$string['group_matrix_operators']       = 'Matrices';
$string['group_power_root']             = 'Powers and roots';
$string['group_set_theory']             = 'Set theory';
$string['group_statistical_operators']  = 'Statistics';
$string['group_trigonometry']           = 'Trigonometry';
$string['group_vector_differential']    = 'Vector differential';
$string['group_vector_operators']       = 'Vectors';
$string['group_vector_products'] = 'Vector products';
$string['implicitmode_explicit_multi'] = 'insert explicit multiplication, assume multi-character variables (2ab → 2*ab)';
$string['implicitmode_explicit_single'] = 'insert explicit multiplication, assume single-character variables (2ab → 2*a*b)';
$string['implicitmode_space_multi'] = 'separate by spaces, assume multi-character variables (2ab → 2 ab)';
$string['implicitmode_space_single'] = 'separate by spaces, assume single-character variables (2ab → 2 a b)';
$string['implicitmode_stack'] = 'leave untouched, let STACK handle it ("star options")';
$string['insertstars_unknown'] = 'Unknown STACK setting (value {$a})';
$string['integral_integrand_missing'] = 'Incomplete integral: the integrand is missing.';
$string['integral_limit_missing'] = 'Incomplete integral: enter both limits or none.';
$string['integral_variable_composite'] = 'Integration with respect to a composite quantity (d f(x)) is not supported. Use a single variable such as dx or dt.';
$string['integral_variable_missing'] = 'Incomplete integral: the integration variable (dx) is missing.';
$string['matrix_cell_empty'] = 'Incomplete matrix: every cell needs a value before the answer can be used.';
$string['notstackquestion'] = 'This question is not a STACK question.';
$string['pluginname'] = 'STACK MathQuill Editor';
$string['popup_column_vector'] = 'Column vector';
$string['popup_dimension'] = 'Dimension';
$string['popup_matrix_size'] = '{a} × {b} matrix';
$string['popup_matrix_title'] = 'Matrix';
$string['popup_orientation'] = 'Orientation';
$string['popup_row_vector'] = 'Row vector';
$string['popup_vector_size'] = '{a}-dimensional {b}';
$string['popup_vector_title'] = 'Vector';
$string['privacy:metadata:allowed_elements'] = 'JSON toolbar configuration including _enabled and _variableMode flags.';
$string['privacy:metadata:cmid'] = 'The course module ID of the quiz.';
$string['privacy:metadata:local_stackmatheditor'] = 'Stores toolbar configuration per quiz and question (or as a quiz default when questionbankentryid is NULL).';
$string['privacy:metadata:questionbankentryid'] = 'The question bank entry ID (version-independent); NULL = quiz-level default.';
$string['privacy:metadata:timecreated'] = 'When the configuration was created.';
$string['privacy:metadata:timemodified'] = 'When the configuration was last changed.';
$string['privacy:metadata:usermodified'] = 'The person who last modified the configuration.';
$string['questionpreview'] = 'Question preview';
$string['resize_confirm'] = 'This removes {a} filled cells. Continue?';
$string['save'] = 'Save configuration';
$string['setting_allowstudenttoggle'] = 'Allow students to switch the editor on and off';
$string['setting_allowstudenttoggle_desc'] = 'While answering, students see a switch that puts the editor away and brings the plain STACK input back, with their answer. Switch this off to take the choice away everywhere; a quiz or a question can take it away as well, but none of them can give it back.';
$string['setting_coordinateseparator'] = 'Coordinate separator';
$string['setting_coordinateseparator_desc'] = 'How the coordinates of a point are separated on screen and when typing. This is notation only: STACK always receives a list, for example [2,3].';
$string['setting_defaultgroups'] = 'Default toolbar groups';
$string['setting_defaultgroups_desc'] = 'Select the toolbar groups enabled by default. Hold Ctrl/Cmd for multi-select. Can be overridden per quiz or question.';
$string['setting_defaultgroups_help'] = 'Select which toolbar element groups are available in the MathQuill editor. Hold Ctrl (Cmd on Mac) to select multiple groups.';
$string['setting_diffop_curl'] = 'Maxima function for the curl';
$string['setting_diffop_curl_desc'] = 'Name of the Maxima function this site provides for the curl (rotation). Leave empty to hide the button. Maxima’s own curl lives in the vect package and needs express(); many questions define their own.';
$string['setting_diffop_divergence'] = 'Maxima function for the divergence';
$string['setting_diffop_divergence_desc'] = 'Name of the Maxima function this site provides for the divergence. Leave empty to hide the button.';
$string['setting_diffop_gradient'] = 'Maxima function for the gradient';
$string['setting_diffop_gradient_desc'] = 'Name of the Maxima function this site provides for the gradient. Leave empty to hide the button.';
$string['setting_diffop_laplacian'] = 'Maxima function for the Laplace operator';
$string['setting_diffop_laplacian_desc'] = 'Name of the Maxima function this site provides for the Laplace operator. Leave empty to hide the button; a bare Δ then stays the Greek letter.';
$string['setting_enabled'] = 'Plugin activation (instance-wide)';
$string['setting_enabled_desc'] = 'Controls how the visual math editor is enabled across the instance.';
$string['setting_extrapagetypes'] = 'Additional page types';
$string['setting_extrapagetypes_desc'] = 'Page types of further activities that render STACK questions through the Moodle question engine, one per line (for example mod-myactivity-attempt). The editor already runs on quiz, adaptive quiz, question preview, CAPQuiz, StudentQuiz and embedded questions. On a page without a STACK input nothing happens.';
$string['setting_maxstructureddimension'] = 'Maximum matrix / vector dimension';
$string['setting_maxstructureddimension_desc'] = 'The largest number of rows, columns or vector elements the selection popup offers. The default is 5; values between 2 and 20 are accepted. It limits the selection interface only - it changes nothing about STACK and does not shorten an answer that is already there. A quiz or a question can set its own value; leave the field empty there to inherit. Without the matrix or the vector group the setting has no effect.';
$string['setting_maxstructureddimension_help'] = 'The largest number of rows, columns or vector elements the selection popup offers. The default is 5. Leave empty to inherit the value from the level above. Without the matrix or the vector group the setting has no effect.';
$string['setting_normfunction'] = 'Maxima function for a norm';
$string['setting_normfunction_desc'] = 'Maxima function the double bar is written to. The default follows the vector format: with vectors as lists it is STACK\'s own Length(), which takes a list of any dimension; with vectors as matrices there is no such function in STACK and the question has to define one, conventionally norm(v) := sqrt(v . v).';
$string['setting_pointnotation'] = 'Draw lists as points';
$string['setting_pointnotation_desc'] = 'When a stored answer contains a list such as [2,3], show it as a point P(2|3). Off by default, because a list is not always a point: with the vector format set to list it is a vector, and elsewhere it may be an ordinary list.';
$string['setting_usepercentpi']      = 'Use percent-pi notation for π';
$string['setting_usepercentpi_desc'] = 'When enabled, π is sent to Maxima as %pi (compatible with STACK evaluation). When disabled, plain pi is used (compatible with STACK star-options mode).';
$string['setting_vectorformat'] = 'Vectors in Maxima';
$string['setting_vectorformat_desc'] = 'How a one-row or one-column matrix is written to Maxima. Choose the form your STACK questions expect: an answer test comparing a list with a matrix fails even when the entries are identical. A list has no orientation, so in list mode a pre-filled answer always comes back as a row.';
$string['setting_vectorformat_list'] = 'List: [a,b,c]';
$string['setting_vectorformat_matrix'] = 'Matrix: matrix([a,b,c])';
$string['stacksemantics'] = 'STACK input semantics';
$string['stacksemantics_desc'] = 'This is STACK\'s “Insert stars” setting for each input of this question. It decides how implicit multiplication and spaces are read. The editor passes on what was typed and does not change this behaviour.';
$string['stacksemantics_edit'] = 'Edit STACK input settings';
$string['toggle_editor'] = 'Formula editor';
$string['toggle_editor_off'] = 'Formula editor off: type the answer as plain text.';
$string['toggle_editor_on'] = 'Formula editor on.';
$string['unit_a']         = 'Ampere (current)';
$string['unit_atm']       = 'Atmosphere (pressure)';
$string['unit_bar']       = 'Bar (pressure)';
$string['unit_c_coulomb'] = 'Coulomb (charge)';
$string['unit_cal']       = 'Calorie (energy)';
$string['unit_cm']        = 'Centimetre (length)';
$string['unit_dl']        = 'Decilitre (volume)';
$string['unit_ev']        = 'Electron volt (energy)';
$string['unit_f']         = 'Farad (capacitance)';
$string['unit_ft']        = 'Foot (length)';
$string['unit_g']         = 'Gram (mass)';
$string['unit_ghz']       = 'Gigahertz (frequency)';
$string['unit_h']         = 'Hour (time)';
$string['unit_hr']        = 'Hour (time)';
$string['unit_hz']        = 'Hertz (frequency)';
$string['unit_j']         = 'Joule (energy)';
$string['unit_k']         = 'Kelvin (temperature)';
$string['unit_kcal']      = 'Kilocalorie (energy)';
$string['unit_kg']        = 'Kilogram (mass)';
$string['unit_khz']       = 'Kilohertz (frequency)';
$string['unit_kj']        = 'Kilojoule (energy)';
$string['unit_km']        = 'Kilometre (length)';
$string['unit_kn']        = 'Kilonewton (force)';
$string['unit_kpa']       = 'Kilopascal (pressure)';
$string['unit_kv']        = 'Kilovolt (voltage)';
$string['unit_kw']        = 'Kilowatt (power)';
$string['unit_l']         = 'Litre (volume)';
$string['unit_lb']        = 'Pound (mass)';
$string['unit_m']         = 'Metre (length)';
$string['unit_ma']        = 'Milliampere (current)';
$string['unit_mg']        = 'Milligram (mass)';
$string['unit_mhz']       = 'Megahertz (frequency)';
$string['unit_mi']        = 'Mile (length)';
$string['unit_min']       = 'Minute (time)';
$string['unit_mj']        = 'Megajoule (energy)';
$string['unit_ml']        = 'Millilitre (volume)';
$string['unit_mm']        = 'Millimetre (length)';
$string['unit_mol']       = 'Mole (amount of substance)';
$string['unit_mpa']       = 'Megapascal (pressure)';
$string['unit_ms']        = 'Millisecond (time)';
$string['unit_mw']        = 'Megawatt (power)';
$string['unit_n']         = 'Newton (force)';
$string['unit_nm']        = 'Nanometre (length)';
$string['unit_ohm']       = 'Ohm (resistance)';
$string['unit_oz']        = 'Ounce (mass)';
$string['unit_pa']        = 'Pascal (pressure)';
$string['unit_s']         = 'Second (time)';
$string['unit_t']         = 'Tonne (mass)';
$string['unit_um']        = 'Micrometre (length)';
$string['unit_v']         = 'Volt (voltage)';
$string['unit_w']         = 'Watt (power)';
$string['unit_yd']        = 'Yard (length)';
