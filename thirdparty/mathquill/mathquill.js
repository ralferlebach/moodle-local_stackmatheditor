...
(function () {
    ...
    function hasSpacingBug() {
        if (_hasSpacingBug === undefined)
            _hasSpacingBug = computeHasSpacingBug();
    ...
    function computeHasSpacingBug() {
        ...
        var lastChild = makeChild(parent);
        document.body.appendChild(parent);
        var diff = parent.getBoundingClientRect().right -
    ...
    function getInterface(v) {
        ...
        var AbstractMathQuill = /** @class */ (function (_super) {
            ...
            AbstractMathQuill.prototype.domNodeToSpan = function (dom) {
        ...
        var EditableField = /** @class */ (function (_super) {
            ...
            EditableField.prototype.insertMatrix = function (optionsOrRows, columns, environment) {
                var options = normalizeMatrixInsertOptions(optionsOrRows, columns, environment);
                var ctrlr = this.__controller.notify(undefined);
                var cursor = ctrlr.cursor;
                if (cursor.isTooDeep())
                    return this;
                var matrix = new Matrix(options.environment, options.rows, options.columns);
                if (cursor.selection) {
                    var replaced = cursor.replaceSelection();
                    if (replaced)
                        matrix.replaces(replaced);
                }
            ...
            EditableField.prototype.insertRowVector = function (columns, environment) {
    ...
    var saneKeyboardEvents = (function () {
        ...
        textarea, controller) {
            ...
            function onPaste(e) {
                ...
                var clipboardEvent = e instanceof ClipboardEvent ? e : undefined;
                if (clipboardEvent && ((_c = controller.options) === null || _c === void 0 ? void 0 : _c.overridePaste)) {
                    var earlyReturn = controller.options.overridePaste(clipboardEvent);
                    if (earlyReturn)
                        return;
                }
            ...
            function updateClipboardData(e) {
                if (e === null || e === void 0 ? void 0 : e.clipboardData) {
                    var selection = controller.exportLatexSelection().selection;
                    if (selection.startIndex !== selection.endIndex) {
                        var text = selection.latex.slice(selection.startIndex, selection.endIndex);
            ...
            if (controller.options && controller.options.disableCopyPaste) {
                ...
                controller.addTextareaEventListeners({
                    ...
                    cut: function (evt) {
                        var _c;
                        var clipboardEvent = evt instanceof ClipboardEvent ? evt : undefined;
                        if (clipboardEvent && ((_c = controller.options) === null || _c === void 0 ? void 0 : _c.overrideCut)) {
                            var earlyReturn = controller.options.overrideCut(clipboardEvent);
                            if (earlyReturn)
                                return;
                        }
                    ...
                    copy: function (evt) {
                        var _c;
                        var clipboardEvent = evt instanceof ClipboardEvent ? evt : undefined;
                        if (clipboardEvent && ((_c = controller.options) === null || _c === void 0 ? void 0 : _c.overrideCopy)) {
                            var earlyReturn = controller.options.overrideCopy(clipboardEvent);
                            if (earlyReturn)
                                return;
                        }
                        updateClipboardData(clipboardEvent);
    ...
    var Controller_latex = /** @class */ (function (_super) {
        ...
        Controller_latex.prototype.domNodeToMqNode = function (dom) {
            ...
            while (dom) {
                ...
                var node = NodeBase.getNodeOfElement(dom);
                if (node) {
        ...
        Controller_latex.prototype.domNodeToSpan = function (dom) {
            var ctx = {
                uncleanedLatex: '',
                uncleanedStartIndex: -1,
                uncleanedEndIndex: -1
            };
            var mqNode = this.domNodeToMqNode(dom);
            if (!mqNode)
                return undefined;
    ...
    var askIfShouldIgnoreMousemoveNoop = function (_evt, _el) {
        return false;
    };
    ...
    var Controller_mouse = /** @class */ (function (_super) {
        ...
        function Controller_mouse() {
            ...
            _this_1.handleMouseDown = function (e) {
                ...
                ctrlr
                    ...
                    .cursor.startSelection();
                var originalSelection = ctrlr.exportLatexSelection().selection;
        ...
        Controller_mouse.prototype.seek = function (targetElm, clientX, _clientY) {
            ...
            var node = this.domNodeToMqNode(targetElm);
    ...
    var MathBlock = /** @class */ (function (_super) {
        ...
        MathBlock.prototype.mathspeak = function () {
            ...
            var speechArray = this.foldChildren([], function (speechArray, cmd) {
    ...
    */
    ...
    var Token = /** @class */ (function (_super) {
        ...
        Token.prototype.latexRecursive = function (ctx) {
            this.checkCursorContextOpen(ctx);
            ctx.uncleanedLatex += "".concat(this.ctrlSeq, "{") + this.tokenId + '}';
            this.checkCursorContextClose(ctx);
        };
    ...
    function isMatrixEnvironmentName(name) {
    ...
    var MatrixCell = /** @class */ (function (_super) {
        ...
        MatrixCell.prototype.moveOutOf = function (dir, cursor, _updown) {
            _super.prototype.moveOutOf.call(this, dir, cursor);
        };
    ...
    var Matrix = /** @class */ (function (_super) {
        ...
        function Matrix(environment, rows, columns) {
            if (environment === void 0) { environment = 'matrix'; }
            if (rows === void 0) { rows = 2; }
            if (columns === void 0) { columns = 2; }
            var _this_1 = _super.call(this) || this;
            _this_1.cells = [];
            _this_1.rowCount = 0;
            _this_1.columnCount = 0;
            _this_1.environment = environment;
            _this_1.ctrlSeq = '\\begin{' + environment + '}';
            _this_1.initialRows = rows;
            _this_1.initialColumns = columns;
            _this_1.ariaLabel = 'matrix';
            return _this_1;
        }
        //
        // -*- Structure -*-
        //
        Matrix.prototype.numBlocks = function () {
            return this.blocks
                ? this.blocks.length
        ...
        Matrix.prototype.createBlocks = function () {
            var cells = [];
            for (var r = 0; r < this.initialRows; r += 1) {
                var row = [];
                for (var c = 0; c < this.initialColumns; c += 1) {
        ...
        Matrix.prototype.adoptCells = function (cells) {
         ...
         */
        Matrix.prototype.syncCells = function () {
            var _this_1 = this;
            var cells = [];
            var blocks = [];
        ...
        Matrix.prototype.relink = function () {
            ...
            if (table) {
                if (cells.length === 1)
                    table.classList.add('mq-rows-1');
                else
                    table.classList.remove('mq-rows-1');
            }
        };
        Matrix.prototype.finalizeTree = function () {
            this.syncCells();
            this.relink();
        };
        //
        ...
        //
        Matrix.prototype.parser = function () {
            var self = this;
            var optWhitespace = Parser.optWhitespace;
            var string = Parser.string;
            return optWhitespace
        ...
        Matrix.prototype.latexRecursive = function (ctx) {
            this.checkCursorContextOpen(ctx);
        ...
        Matrix.prototype.mathspeak = function () {
            var parts = [
        ...
        Matrix.prototype.html = function () {
            var delimiters = MATRIX_ENVIRONMENTS[this.environment];
            var leftSymbol = matrixSymbol(delimiters.left);
            var rightSymbol = matrixSymbol(delimiters.right);
            var columnCount = this.columnCount;
            var blocks = this.blocks || [];
            this.domView = new DOMView(blocks.length, function (renderedBlocks) {
                var trs = [];
                var rowCount = Math.ceil(renderedBlocks.length / columnCount);
                for (var r = 0; r < rowCount; r += 1) {
                    var tds = [];
                    for (var c = 0; c < columnCount; c += 1) {
                        var block = renderedBlocks[r * columnCount + c];
                        if (!block)
                            continue;
                ...
                var table = h('table', {
                    class: 'mq-matrix-table mq-non-leaf' + (rowCount === 1 ? ' mq-rows-1' : '')
                }, [h('tbody', {}, trs)]);
                var children = [];
                if (leftSymbol) {
                ...
                return h('span', { class: 'mq-matrix mq-non-leaf mq-bracket-container' }, children);
            });
            return _super.prototype.html.call(this);
        };
        Matrix.prototype.tableElement = function () {
        ...
        Matrix.prototype.rowElements = function () {
            var table = this.tableElement();
            if (!table)
                return [];
        ...
        Matrix.prototype.newCell = function () {
            var cell = new MatrixCell();
            cell.matrix = this;
            var td = h.block('td', { class: 'mq-matrix-cell mq-empty' }, cell);
        ...
        Matrix.prototype.insertRowAfter = function (afterCell, cursor) {
            var row = afterCell.row;
            var column = afterCell.column;
            var tr = h('tr', {}, []);
            var newCells = [];
            var leftward = this.cells[row][this.columnCount - 1];
            for (var c = 0; c < this.columnCount; c += 1) {
                var created = this.newCell();
                tr.appendChild(created.td);
                created.cell.adopt(this, leftward, leftward[R]);
                leftward = created.cell;
                newCells.push(created.cell);
            }
            var trs = this.rowElements();
            if (trs[row]) {
                var parentNode = trs[row].parentNode;
                if (parentNode)
                    parentNode.insertBefore(tr, trs[row].nextSibling);
            }
            this.syncCells();
            this.relink();
        ...
        Matrix.prototype.insertColumnAfter = function (afterCell, cursor) {
            var column = afterCell.column;
            var row = afterCell.row;
            var trs = this.rowElements();
            var newCells = [];
            for (var r = 0; r < this.rowCount; r += 1) {
                var created = this.newCell();
                var leftward = this.cells[r][column];
                created.cell.adopt(this, leftward, leftward[R]);
                newCells.push(created.cell);
                var tr = trs[r];
                if (tr) {
                    var tds = tr.querySelectorAll('td');
                    var reference = tds[column];
                    if (reference) {
        ...
        Matrix.prototype.deleteRow = function (row) {
            if (this.rowCount <= 1)
                return;
            var trs = this.rowElements();
            for (var c = 0; c < this.cells[row].length; c += 1) {
        ...
        Matrix.prototype.deleteColumn = function (column) {
        ...
        Matrix.prototype.finishMutation = function (cellToFocus, cursor) {
            if (cursor) {
        ...
        Matrix.prototype.isRowEmpty = function (row) {
            for (var c = 0; c < this.cells[row].length; c += 1) {
        ...
        Matrix.prototype.isColumnEmpty = function (column) {
            for (var r = 0; r < this.rowCount; r += 1) {
                if (!this.cells[r][column].isEmpty())
                    return false;
            }
            return true;
        };
         ...
         */
        Matrix.prototype.deleteOutOfCell = function (cell, dir, cursor) {
            if (cell.isEmpty()) {
            ...
            if (neighbour) {
                cursor.insAtDirEnd(-dir, neighbour);
            }
            else {
                cursor.insDirOf(dir, this);
            }
        ...
        Matrix.prototype.neighbourOutsideRow = function (cell, dir) {
            var row = cell.row;
            if (dir === L && row > 0) {
        ...
        Matrix.prototype.neighbourOutsideColumn = function (cell, dir) {
            var row = cell.row;
    ...
    function matrixSymbol(name) {
        if (!name)
            return null;
        return SVG_SYMBOLS[name];
    }
    LatexEnvironments.matrix = function () { return new Matrix('matrix'); };
    LatexEnvironments.pmatrix = function () { return new Matrix('pmatrix'); };
    LatexEnvironments.bmatrix = function () { return new Matrix('bmatrix'); };
    LatexEnvironments.Bmatrix = function () { return new Matrix('Bmatrix'); };
    LatexEnvironments.vmatrix = function () { return new Matrix('vmatrix'); };
    LatexEnvironments.Vmatrix = function () { return new Matrix('Vmatrix'); };
    /**
     * \begin{env} ... \end{env}
     *
    ...
    LatexCmds.begin = /** @class */ (function (_super) {
        ...
        class_15.prototype.parser = function () {
            ...
            return string('{')
                ...
                .skip(string('}'))
                .then(function (env) {
                var factory = LatexEnvironments[env];
                if (!factory) {
     ...
     */
    function normalizeMatrixInsertOptions(optionsOrRows, columns, environment) {
        var options = typeof optionsOrRows === 'number'
            ? {
                rows: optionsOrRows,
                columns: columns,
        ...
        var env = options.environment || 'bmatrix';
    ...
    function validateMatrixDimension(name, value) {
        if (typeof value !== 'number' ||
            !isFinite(value) ||
            Math.floor(value) !== value ||
            value < 1 ||
            value > MATRIX_MAX_DIMENSION) {
            throw new Error('MathQuill: matrix ' +
                name +
